...
export function CartPage() {
  ...
  const handleQuantityChange = useCallback((item: CartItem, delta: number) => {
    ...
    if (delta > 0) {
      showToast(`${item.name} — ahora llevas ${newQty}`);
    } else {
      showToast(`${item.name} — cantidad reducida (${newQty})`);
    }
  }, [updateQuantity, showToast]);

  const handleRemove = useCallback((item: CartItem) => {
    removeFromCart(item.cartItemId);
    showToast(`${item.name} eliminado del carrito`, 'remove');
  }, [removeFromCart, showToast]);

  const handleClearCart = useCallback(() => {
    clearCart();
    showToast('Carrito vaciado', 'remove');
  }, [clearCart, showToast]);
